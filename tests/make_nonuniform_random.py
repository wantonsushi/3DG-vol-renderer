...
def biased_y(low=0.0, high=2.0, power=2.0):
    """Sample y in [low, high], biased toward low end using power transform."""
    u = np.random.uniform(0.0, 1.0)
    return low + (high - low) * (u ** power)

def generate_gaussian():
    """Generate a single Gaussian entry with -y bias."""
    x = np.random.uniform(-1.0, 1.0)
    y = biased_y(0.0, 2.0, power=2.0)  # bias stronger toward 0
...
def generate_scene(num_gaussians, output_filename):
    ...
    lines = ["// Scene with biased Gaussians (-y preference)"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a Gaussian scene file (biased toward -y).")
    parser.add_argument("num_gaussians", type=int, help="Number of Gaussians to generate.")
    parser.add_argument("filename", type=str, help="Output file name (e.g., biased.txt).")
